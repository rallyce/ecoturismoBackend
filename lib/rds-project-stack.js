...
class RdsProjectStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    const paymentLambda = new lambda.Function(this, 'PaymentLambda', {
      functionName: 'ExecutingPayment',
      runtime: lambda.Runtime.NODEJS_18_X,
      handler: 'index.handler',
      code: lambda.Code.fromAsset('lambda/executingPayment'),
      timeout: Duration.seconds(10),
    });

    const endingPayment = new lambda.Function(this, 'EndingPayment', {
      functionName: 'EndingPayment',
      runtime: lambda.Runtime.NODEJS_18_X,
      handler: 'index.handler',
      code: lambda.Code.fromAsset('lambda/endingPayment'),
      environment: {
        DB_HOST: dbInstance.dbInstanceEndpointAddress,
        DB_USER: 'admin',
        DB_PASSWORD: dbCredentialsSecret.secretValueFromJson('password').unsafeUnwrap(),
    ...
    const getApiIntegration4 = new apigateway.LambdaIntegration(paymentLambda);
    const getApiIntegration5 = new apigateway.LambdaIntegration(endingPayment);

    const apiResource1 = api.root.addResource('calculations');
    ...
    const apiResource4 = api.root.addResource('pay');
    apiResource4.addMethod('POST', getApiIntegration4);

    const apiResource5 = api.root.addResource('endingPayment');
    apiResource5.addMethod('POST', getApiIntegration5);
    apiResource5.addMethod('GET', getApiIntegration5);
    apiResource5.addMethod('PUT', getApiIntegration5);
    apiResource5.addMethod('DELETE', getApiIntegration5);
